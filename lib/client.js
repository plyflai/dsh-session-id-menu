...
window.__ModuleLoader__.load({
  ...
  factory: function () {
    ...
    function apply(ctx) {
      ...
      // Live snapshot seam (Phase-2 runtime finding): the 'sessions' and
      // 'workspaces' services (ClientSessions / WorkspaceController) carry
      // their list stores on `.list` — the snapshot is read through the
      // store, not the service (a top-level getSnapshot does not exist; the
      // first live click died with "sessions.getSnapshot is not a function"
      // inside the click listener). A missing or malformed seam degrades to
      // a silent miss instead of an uncaught exception.
      function readListSnapshot(store) {
        var src = store && store.list
        if (!src || typeof src.getSnapshot !== 'function') return null
        try {
      ...
      function resolveSessionId(row) {
        var snap = readListSnapshot(sessions)
        ...
        if (header) {
          ...
          if (htext !== '') {
            var wssnap = readListSnapshot(workspaces)
